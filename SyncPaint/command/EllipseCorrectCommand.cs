...
{
    class EllipseCorrectCommand:ShapeCorrectCommand
    {
        public Point lt, rt, rb, lb;
       public EllipseCorrectCommand(byte[] data):base(ShapeType.ELLIPSES)
        {
            lt = new Point();
            rt = new Point();
            rb = new Point();
        ...
        {
            return "形状修正为 椭圆 " + lt.ToString() + " " + rt.ToString() + " " + rb.ToString() + " " + lb.ToString();
